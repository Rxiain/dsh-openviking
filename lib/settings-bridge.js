...
export function bridgeFailureOf(error) {
 ...
 */
export function makeBridgeHandlers(deps) {
    const view = () => {
    ...
    return {
        ...
        async mutate(request) {
            ...
            try {
                await deps.settings.mutate(settingsNamespace(SERVED_NAMESPACE), body.ops, expectedRevision);
            }
            catch (error) {
