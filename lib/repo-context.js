...
export function createRepoContext(ctx, client, config) {
        ...
        inflight = (async () => {
            try {
                ...
                for (const raw of items) {
                    if (!isRecord(raw) || typeof raw.uri !== "string")
                        continue;
                    const uri = raw.uri;
                    if (uri === "viking://resources/" || !uri.startsWith("viking://resources/"))
                        continue;
    ...
    function getPrompt() {
        if (!getConfig().enabled || !cachedRepos)
            return "";
        return [
...
function formatRepoLine(item) {
    ...
    const abstract = typeof item.abstract === "string" ? item.abstract : typeof item.overview === "string" ? item.overview : "";
