...
export function createMemoryRecall(ctx, client, config) {
    ...
    function hasRenderableLeaf(items, scoreThreshold) {
        return items.some((item) => isLeafLikeMemory(item) && hasMemoryText(item) && recallClampScore(item.score) >= scoreThreshold);
    }
...
function pickMemoriesForInjection(items, limit, queryText, scoreThreshold = 0) {
    ...
    const leaves = deduped.filter((item) => isLeafLikeMemory(item) && recallClampScore(item.score) >= scoreThreshold);
    const picked = [...leaves];
