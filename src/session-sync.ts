...
interface PersistedSessionState {
  syncedMessageIds?: unknown;
  uncommittedMessageIds?: unknown;
...
interface SessionState {
  syncedMessageIds: Set<string>;
  uncommittedMessageIds: Set<string>;
...
/** What a serialized commit chain task decided; the poll happens off-chain. */
type CommitOutcome =
  | { kind: "pending"; taskId: string }
  | { kind: "posted"; taskId: string; archived: boolean }
  | { kind: "synced"; archived: boolean };
...
function isEligibleEvent(event: SessionEvent): boolean {
 ...
 */
export function stripRecallBlock(text: string): string {
...
export class SessionManager {
  ...
  private readonly drainScheduled = new Set<string>();
  ...
  private autoCommitRunning = false;
  ...
  async init(): Promise<void> {
    try {
      ...
      for (const agent of this.ctx.agents.list()) this.adoptDirect(agent);
  ...
  private adoptDirect(agent: Agent): void {
  ...
  private stateFor(key: string): SessionState {
   ...
   */
  async ensureSession(agent: Agent, signal?: AbortSignal): Promise<void> {
    const key = String(agent.id);
   ...
   * never interleave.
   */
  private runOnChain<T>(key: string, fn: () => Promise<T>): Promise<T> {
   ...
   */
  queueDrain(agent: Agent, options: { ensure?: boolean } = {}): void {
    const key = String(agent.id);
    if (this.closing || this.drainScheduled.has(key)) return;
    this.drainScheduled.add(key);
    void this.runOnChain(key, async () => {
      this.drainScheduled.delete(key);
      try {
        if (options.ensure) await this.ensureSession(agent);
  ...
  async waitForChain(agent: Agent): Promise<void> {
  ...
  async drainAgentNow(agent: Agent, signal?: AbortSignal): Promise<void> {
    ...
    for (const event of agent.session.snapshotEvents()) {
      ...
      if (state.syncedMessageIds.has(dedupeKey)) continue;
   ...
   */
  async flushSession(agent: Agent, signal?: AbortSignal): Promise<void> {
   ...
   */
  async commitCurrentSession(agent: Agent, signal?: AbortSignal, explicitSessionId?: string): Promise<CommitToolResult> {
    const key = String(agent.id);
    ...
    const outcome = await this.runOnChain(key, async (): Promise<CommitOutcome> => {
      ...
      // survive.
      for (const id of snapshot) state.uncommittedMessageIds.delete(id);
   ...
   */
  async commitExplicitSession(sessionId: string, signal?: AbortSignal): Promise<CommitToolResult> {
    const commit = await this.client.commitSession(sessionId, signal);
    if (typeof commit.task_id !== "string" || !commit.task_id) {
      return commitToolResult({
        session_id: sessionId,
  ...
  private async pollTask(taskId: string, signal: AbortSignal | undefined, timeoutMs: number): Promise<TaskResultLike | undefined> {
  ...
  private async pollPendingOnce(key: string, state: SessionState): Promise<void> {
  ...
  private startAutoCommit(): void {
    if (this.autoCommitTimer || !this.config.autoCommit.enabled) return;
  ...
  private async autoCommitTick(): Promise<void> {
    if (this.autoCommitRunning) return;
    ...
    try {
      ...
      for (const [key, state] of this.states) {
        ...
        await this.runOnChain(key, async () => {
          ...
          if (this.backgroundAbort.signal.aborted) return;
          if (state.uncommittedMessageIds.size === 0) return;
   ...
   */
  private async postCommit(state: SessionState, sessionId: string, signal: AbortSignal): Promise<void> {
    const snapshot = [...state.uncommittedMessageIds];
    const commit = await this.client.commitSession(sessionId, signal);
    if (typeof commit.task_id === "string" && commit.task_id) {
      state.pendingCommit = { taskId: commit.task_id, startedAt: Date.now(), messageIds: snapshot };
  ...
  private async loadState(): Promise<void> {
    ...
    const persisted = parsed as PersistedState & { identity: Record<string, unknown>; sessions: Record<string, PersistedSessionState> };
    const current = {
      endpoint: normalizeEndpoint(this.config.endpoint),
      account: this.config.account,
      user: this.config.user,
    ...
    const identity = persisted.identity as Record<string, unknown>;
    if (
      identity.endpoint !== current.endpoint ||
    ...
    for (const [key, entry] of Object.entries(persisted.sessions)) {
      const state: SessionState = {
        syncedMessageIds: new Set(
          Array.isArray(entry.syncedMessageIds) ? entry.syncedMessageIds.filter((v): v is string => typeof v === "string") : [],
        ),
        uncommittedMessageIds: new Set(
          Array.isArray(entry.uncommittedMessageIds)
            ? entry.uncommittedMessageIds.filter((v): v is string => typeof v === "string")
            : [],
        ),
      };
      if (typeof entry.lastCommitTime === "number") state.lastCommitTime = entry.lastCommitTime;
      if (isRecord(entry.pendingCommit)) {
        const pending = entry.pendingCommit as Record<string, unknown>;
        if (typeof pending.taskId === "string" && Array.isArray(pending.messageIds)) {
          state.pendingCommit = {
            ...
            messageIds: pending.messageIds.filter((v): v is string => typeof v === "string"),
          };
        }
      }
      this.states.set(key, state);
  ...
  private validateShape(parsed: unknown): parsed is { version: 1; identity: unknown; sessions: unknown } {
    return (
      isRecord(parsed) &&
      parsed.version === 1 &&
  ...
  private async quarantine(suffix: string): Promise<void> {
    try {
  ...
  private debouncedSave(): void {
    if (this.closing) return; // the disposer's final save covers everything
  ...
  private warnOnce(operation: string, key: string, error: unknown): void {
    const dedupeKey = `${operation}:${this.client.endpoint}:${messageOf(error)}`;
    if (this.warningKeys.has(dedupeKey)) return;
  ...
  async dispose(): Promise<void> {
    ...
    const finalDrains: Promise<void>[] = [];
    for (const [key, agent] of [...this.agents]) {
      finalDrains.push(
        this.runOnChain(key, async () => {
          await this.ensureSession(agent, this.backgroundAbort.signal);
          await this.drainAgentNow(agent, this.backgroundAbort.signal);
        }),
      );
...
function totalMemoriesExtracted(memories: unknown): number {
  if (typeof memories === "number") return memories;
  if (!isRecord(memories)) return 0;
