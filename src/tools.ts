...
// ─── memlearn ───────────────────────────────────────────────────────────
//
// The eleventh tool: a deliberate "learn" channel (ported from oh-my-pi's
...
function deferLearned(
  exec: { deferContext(context: unknown): void },
  kind: "skill" | "memory",
  uri: string,
  text: string,
): void {
  ...
  const memlearn = defineTool({
    name: "memlearn",
    description:
      "Deliberately capture a reusable lesson into OpenViking memory or mint/update a skill playbook. " +
      ...
      skill: {
        ...
        properties: {
          action: {
            ...
            enum: ["create", "update"],
            description:
              "Intent hint. `create` requires the skill to not exist yet; `update` requires it to already exist. " +
    ...
    output: {
      ...
      render: (_args, value: {
        action: string; kind: string; uri: string; score?: number; redacted: number; injected: boolean; message: string;
      }) => jsonText(value),
    },
    timeoutMs: 30_000,
    async execute(args, exec) {
